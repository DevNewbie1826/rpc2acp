...
if (process.argv.includes('--terminal-login')) {
  ...
  if ((res as any).error && (res as any).error.code === 'ENOENT') {
    process.stderr.write(
      `rpc2acp: could not start ${variant.command} (command not found: ${cmd}). Install it via \`npm install -g ${variant.npmPackage}\` or ensure \`${variant.command}\` is on your PATH.\n`
    )
    process.exit(1)
  }
