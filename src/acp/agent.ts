...
import { resolveVariant, getUserPromptsDir, getExtensionsDir, getProjectSkillsDir, getGlobalSettingsPath } from './agent-variant.js'
...
export class PiAcpAgent implements ACPAgent {
  ...
  constructor(conn: AgentSideConnection, _config?: unknown) {
    this.sessions.startIdleSweeper()
    this.conn = conn
    void _config
  }
    ...
    return {
      ...
      agentCapabilities: {
        ...
        sessionCapabilities: {
          ...
          list: {},
          // Multiple sessions stay live concurrently; clients release them via session/close.
          close: {}
        }
      }
    }
      ...
      if (cmd === 'changelog') {
        ...
        const findChangelog = (): string | null => {
          ...
          try {
            ...
            const which = spawnSync(whichCmd, [variant.command], { encoding: 'utf-8' })
          ...
          try {
            ...
            if (root) {
              const p = join(root, ...variant.npmPackage.split('/'), 'CHANGELOG.md')
              if (existsSync(p)) return p
            }
          } catch {
            // ignore
  ...
  async closeSession(params: CloseSessionRequest): Promise<CloseSessionResponse> {
    // Cancel any in-flight turn, then release the pi subprocess.
    const session = this.sessions.maybeGet(params.sessionId)
    if (session) {
      try {
        await session.cancel()
...
function buildUpdateNotice(): string | null {
  ...
  try {
    const piVersion = spawnSync(variant.command, ['--version'], { encoding: 'utf-8' })
    ...
    const latestRes = spawnSync('npm', ['view', variant.npmPackage, 'version'], {
      encoding: 'utf-8',
      timeout: 800
    })
    ...
    return `New version available: v${latest} (installed v${installed}). Run: \`npm i -g ${variant.npmPackage}\``
  } catch {
    return null
  }
}
...
function buildStartupInfo(opts: {
  ...
  // Project skills (variant configDir/skills)
  const projectSkillsDir = getProjectSkillsDir(variant, opts.cwd)
  ...
  const promptsDir = getUserPromptsDir(variant)
  try {
  ...
  const extDir = getExtensionsDir(variant)
  try {
  ...
  try {
    const settingsPath = getGlobalSettingsPath(variant)
