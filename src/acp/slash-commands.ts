...
import { getUserPromptsDir, getProjectPromptsDir, resolveVariant } from './agent-variant.js'
...
/**
 * Load prompt templates from the variant's prompt directories (formerly "commands").
 *  - user:    agent prompts dir (recursive, .md files)
 *  - project: project prompts dir (recursive, .md files)
 */
export function loadSlashCommands(cwd: string): FileSlashCommand[] {
  const commands: FileSlashCommand[] = []

  const userDir = getUserPromptsDir(variant)
  const projectDir = getProjectPromptsDir(variant, cwd)
