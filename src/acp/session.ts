...
export class SessionManager {
  ...
  // Idle TTL: sessions with no activity for this long are automatically closed.
  private readonly idleTtlMs: number
  private readonly lastActivity = new Map<string, number>()
  private idleSweeper: NodeJS.Timeout | null = null

  constructor(idleTtlMs?: number) {
    // Default 30 minutes; override via PI_ACP_IDLE_TTL_MS env or constructor arg.
    const envMs = Number(process.env.PI_ACP_IDLE_TTL_MS)
    this.idleTtlMs = idleTtlMs ?? (Number.isFinite(envMs) && envMs > 0 ? envMs : 30 * 60 * 1000)
  }

  /** Start the periodic idle-session sweeper. Call once after construction. */
  startIdleSweeper(intervalMs = 60_000): void {
    if (this.idleSweeper) return
    this.idleSweeper = setInterval(() => this.sweepIdleSessions(), intervalMs)
    // Don't keep the process alive just for the sweeper.
    if (typeof this.idleSweeper === 'object' && 'unref' in this.idleSweeper) {
      this.idleSweeper.unref()
    }
  }

  /** Stop the idle sweeper (e.g. on shutdown). */
   ...
   * Used by session/close, idle TTL, and explicit reload.
   */
  ...
  async create(params: SessionCreateParams): Promise<PiAcpSession> {
    // Let the agent manage session persistence in its default location
    // so sessions are visible to the regular CLI.
