...
### N ACP sessions ↔ N pi subprocesses (one per session)

Pi RPC mode is effectively single-session (no session addressing in the protocol), so the adapter maps each ACP session to a dedicated pi subprocess:

- `session/new` → spawn a dedicated `pi --mode rpc` process
- `session/load` → spawn a dedicated process attached to the persisted session file
- `session/prompt` → send `{type:"prompt"}` to that session's process and stream events back as `session/update`
- `session/cancel` → send `{type:"abort"}` to that session's process
- `session/close` → cancel in-flight work, then terminate that session's process

Sessions coexist concurrently. Lifecycle is client-managed (`session/close`), with a safety-net idle TTL (default 30 min, `PI_ACP_IDLE_TTL_MS`) that closes inactive sessions; the next access transparently re-spawns the subprocess.
